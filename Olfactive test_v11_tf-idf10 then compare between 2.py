...
def build_similarity_matrix(df: pd.DataFrame, p_col: str,
    ...
    """
    Fit vectorizer on ALL fragrances, transform each fragrance document,
    then compute the full cosine similarity matrix.

    Returns (sim_matrix, frag_list) so Tab 2 can do any pairwise lookup
    without re-fitting the vectorizer each time.

    use_tfidf=True  → TF-IDF: IDF from full corpus, common words down-weighted
    use_tfidf=False → CountVectorizer: raw word counts
    """
    ...
    try:
        ...
        mat = vec.fit_transform(docs)     # fit + transform all fragrances at once
...
if "processed_df" in st.session_state:
    ...
    with tab2:
        ...
        st.caption(
            "IDF is fitted on all fragrances in the file, then each pair is compared. "
            "With 2+ candidates: scores stretched to 20–90% for visual spread. "
            "With 1 candidate: raw cosine shown (cross-test comparable)."
        )
        ...
        if cand_sel:
            # ── build full similarity matrix (fit on all frags) ───────────
            ...
            if sim_matrix is None:
                ...
                for cand in cand_sel:
                    cand_idx = frag_list.index(cand)
                    raw_scores.append(float(sim_matrix[ref_idx, cand_idx]))
                ...
                st.pyplot(generate_word_cloud(
                    df[df[p_col].astype(str) == ref_p]["tokens"],
                    palette_opt, shape_opt))

                if len(cand_sel) <= 4:
                    st.write("#### 🧪 Candidates")
                    wc_cols = st.columns(min(len(cand_sel), 2))
                    for i, (cand, _, _) in enumerate(results):
                        with wc_cols[i % 2]:
                            st.write(f"**{cand}**")
                            st.pyplot(generate_word_cloud(
