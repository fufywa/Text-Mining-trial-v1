...
if "processed_df" in st.session_state:
    ...
    with tab2:
        ...
        if not d_a.empty and not d_b.empty:
            sim = compute_raw_similarity(d_a, d_b, df["token_str"])
            st.metric(
                "🔤 Olfactive Similarity",
                f"{round(sim * 100, 1)}%",
                help="Full-corpus TF-IDF (sublinear) cosine similarity. "
                     "High-frequency words shared by all fragrances are down-weighted "
                     "to amplify real differences."
            )
