...
# --- Spell correction: protected vocabulary ---
SPELL_PROTECT: set = {
...
if "ss_spell_protect" not in st.session_state:
    st.session_state["ss_spell_protect"] = set(SPELL_PROTECT)


# =============================================================================
# ▌BLOCK 2 — PIPELINE STEPS
# =============================================================================
...
def process_verbatim(raw_text: str, category: str = None) -> List[str]:
    ...
    tokens = step3_5_spell_correct(tokens)
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            st.divider()
            category_options = ["None"] + sorted(CATEGORY_STOPS.keys())
            cat_choice = st.selectbox(
                "Product Category (for category stopwords)",
                category_options,
                index=category_options.index(st.session_state["ss_category"])
                      if st.session_state["ss_category"] in category_options else 0,
            )
            st.session_state["ss_category"] = cat_choice
    ...
    if uploaded_file and st.button("🚀 Run Analysis"):
        ...
        selected_cat = st.session_state["ss_category"]
        cat_arg = None if selected_cat == "None" else selected_cat

        with st.spinner("Running pipeline…"):
            df_filtered["tokens"]    = df_filtered[v_col].apply(
                lambda x: process_verbatim(x, category=cat_arg))
...
if "processed_df" in st.session_state:
    ...
    with tab5:
        ...
        with col_p2:
            st.write("**Spell Correction Protect**")
            spell_protect_text = st.text_area(
                "Words never corrected by spell checker (comma-separated)",
                value=", ".join(sorted(st.session_state["ss_spell_protect"])),
                height=100,
                key="spell_protect_textarea"
            )
        ...
        if st.button("💾 Apply & Re-Process"):
            ...
            st.session_state["ss_spell_protect"] = {
                x.strip().lower() for x in spell_protect_text.split(",") if x.strip()
            }
            if "processed_df" in st.session_state:
                df_reprocess = st.session_state["processed_df"].copy()
                v_col_r  = st.session_state["v_col"]
                cat_arg  = None if st.session_state["ss_category"] == "None" \
                           else st.session_state["ss_category"]
