...
# Sentence-BERT — loaded lazily on first use to avoid slowing app startup
...
def compute_raw_similarity(token_str_a: pd.Series, token_str_b: pd.Series,
    ...
    """
    Cosine similarity in full-corpus vocabulary space.
    sublinear_tf dampens very frequent shared words to amplify real differences.
    min_df=2 drops words unique to only one fragrance.
    """
    ...
    try:
        vec = TfidfVectorizer(
            ...
            min_df=2,            # ignore words unique to only 1 fragrance
        )
        vec.fit(corpus)                          # vocabulary from ALL fragrances
...
def compute_bert_similarity(texts_a: list, texts_b: list) -> float:
    """
    Sentence-BERT cosine similarity.
    Each fragrance is represented by the mean embedding of its raw verbatims.
    Uses all-MiniLM-L6-v2 (fast, 384-dim, good semantic quality).
    """
    try:
        model  = load_sbert()
        emb_a  = model.encode(texts_a, convert_to_numpy=True, show_progress_bar=False)
        emb_b  = model.encode(texts_b, convert_to_numpy=True, show_progress_bar=False)
        ...
        return None  # caller checks for None and shows warning
...
if "processed_df" in st.session_state:
    ...
    with tab2:
        ...
        if not d_a.empty and not d_b.empty:
            ...
            raw_a = df[df[p_col].astype(str) == p_a][v_col].dropna().tolist()
            raw_b = df[df[p_col].astype(str) == p_b][v_col].dropna().tolist()

            with st.spinner("Computing BERT embeddings…"):
                bert_sim = compute_bert_similarity(raw_a, raw_b)

            m1, m2 = st.columns(2)
            if bert_sim is not None:
                m1.metric(
                    "🧠 Semantic Similarity (BERT)",
                    f"{round(bert_sim * 100, 1)}%",
                    help="Sentence-BERT (all-MiniLM-L6-v2) cosine similarity. "
                         "Each fragrance = mean embedding of all its raw verbatims. "
                         "Captures true semantic proximity regardless of exact wording."
                )
            else:
                m1.warning("BERT model unavailable — install sentence-transformers.")

            # ── TF-IDF similarity (reference) ────────────────────────────
            tfidf_sim = compute_raw_similarity(d_a, d_b, df["token_str"])
            m2.metric(
                "🔤 Token Similarity (TF-IDF)",
                f"{round(tfidf_sim * 100, 1)}%",
                help="Full-corpus sublinear TF-IDF cosine similarity on processed tokens."
            )
