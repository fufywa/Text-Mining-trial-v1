...
if "processed_df" in st.session_state:
    ...
    with tab2:
        ...
        st.caption(
            "Compare candidates against a fixed reference fragrance. "
            "With 2+ candidates: scores are stretched to 20–90% to maximise visual spread "
            "(relative ranking only). With 1 candidate: raw cosine similarity is shown "
            "(absolute value, comparable across tests)."
        )
        ...
        use_stretch = st.toggle(
            ...
            value=True,
            help="ON: scores stretched to 20–90% for visual spread — use for relative ranking "
                 "within a single test. OFF: raw cosine similarity — use for cross-test comparison."
        )
